  class HomeworksController < ApplicationController
    ...
    def share
      @idhomework = Homework.find_by(id: params[:id])
      email = User.find_by_email(params[:user_emails])
      if email
        @taskuser = TaskUser.new(user_id: email.id, homework_id: @idhomework.id)
        if @taskuser.save
          flash[:success] = "The new homework was shared with success."
          redirect_to homeworks_path
        else
          @homework = Homework.new
          @homework.tasknotebooks.build
        ...
        @homework.taskusers.build
        @notebooks = Notebook.where(user_id: current_user.id)
        flash[:error] = "Error usuario no encontrado."
        render 'new'
      end
    end
